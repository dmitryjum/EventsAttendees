...
describe V1::EventsController do
  ...
  context "it gets a list of events" do
    ...
    it "receives response as the object with 'records', 'entries_count', 'pages_per_limit', 'page' keys" do
      ...
      expect(json_response.keys).to eq %w[records entries_count pages_per_limit page]
    end
  ...
  context "it gets an event by id" do
    ...
    it "finds an event by friendly id" do
      get v1_event_path(id: "event-one")
      expect(json_response["id"]).to eq event1.id
    end
  ...
  context "it creates a new event" do
    it "successfuly creates a new event" do
      post v1_events_path(event: { name: "New Modern Event", description: "modern special secret event",
                                   event_type: "in_person" })
      expect(response.status).to be 201
      expect(Event.count).to be 4
      expect(json_response["description"]).to eq(Event.find_by(name: "New Modern Event").description)
    end

    it "fails name validation" do
      post v1_events_path(event: { name: event3.name, description: "event with duplicate name",
                                   event_type: "in_person" })
